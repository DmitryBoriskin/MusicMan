...
+function ($) {
    ...
    inputFile.prototype.defaults = function () {
        return {
            ...
            url: this.$element.attr('value') || inputFile.DEFAULTS.url
        }
    }
    ...
    inputFile.prototype.render = function () {
        ...
        var uploadId = 'posted_' + objName.substring(objName.lastIndexOf('.') + 1);

        this.$element.prop('id', uploadId);
        this.$element.prop('name', uploadId);
        ...
        var fName = this.$element.attr('value');
    ...
    inputFile.prototype.create = function () {
        ...
        if (this.$element.attr('value')) {
            var $img = $("<img/>", { "src": this.$element.attr('value') });
    ...
    $(document).on('change.bs.inputFile', 'input:file:not(.noplagin)', function (e) {
        ...
        //if (fileType.indexOf('image') > -1) {
            $(this).attr('value', window.URL.createObjectURL(File));
        //}
